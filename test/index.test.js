...
describe("Routes", () => {
  ...
  test("add with hideDefaultLocale", () => {
    const routes = nextRoutes({ locale: "en", hideDefaultLocale: true }).add({
      name: "a",
      locale: "en",
      pattern: "/a"
    });
  ...
  test("add with object and data", () => {
    ...
    setup({ name: "a", locale: "en", data: data }).testRoute({
      name: "a",
      locale: "en",
      pattern: "/a",
      page: "/a",
  ...
  test("match and merge params into query", () => {
    const routes = nextRoutes()
      ...
      .add("b", "en", "/b/:b")
      .add("c", "en");
    expect(routes.match("/en/b/b?b=x&c=c").query).toMatchObject({
      b: "b",
      c: "c"
    });
  });
  ...
  test("match homepage route with hideDefaultLocale", () => {
    const routes = nextRoutes({ locale: "cs", hideDefaultLocale: true }).add(
      "homepage",
      "cs",
      ""
    );
  ...
  test("generate urls from params", () => {
    ...
    const expected = {
      as: "/en/a/b/1/2?d=d",
      href: "/a?b=b&c=1%2F2&d=d&nextRoute=a"
    };
    expect(route.getUrls(params)).toEqual(expected);
    expect(setup("a", "en").route.getUrls()).toEqual({
      as: "/en/a",
      href: "/a?nextRoute=a"
    });
  });
...
describe("Link", () => {
  const setup = (...args) => {
    ...
    const testLinkException = addProps => {
      expect(() =>
        renderer.render(<Link {...props} {...addProps} />)
      ).toThrow();
    };
