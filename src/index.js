...
class Routes {
  ...
  add(name, locale = this.locale, pattern, page, data, update = false) {
    ...
    options.hideLocale =
      !!this.hideDefaultLocale && options.locale === this.defaultLocale;
  ...
  setRoutes(routes) {
    if (Array.isArray(routes)) {
      ...
      throw new Error(
        "Data passed to setRoutes is neither an array nor an object"
      );
    }
  }
    ...
    return this.routes.reduce(
      (result, route) => {
        ...
        return {
          ...
          route,
          params,
          query: { ...query, ...params, nextRoute: route.name }
        };
      },
      { query, parsedUrl }
    );
  ...
  getRouter(Router) {
    const wrap = method => (route, params, locale, options) => {
      const {
        byName,
        urls: { as, href }
      } = this.findAndGetUrls(route, locale, params);
...
class Route {
  ...
  match(path) {
    if (
      !this.hideLocale &&
      path.substring(1, this.locale.length + 1) === this.locale
    ) {
